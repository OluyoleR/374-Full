...
trap "rm -rf $out;" EXIT 
{
  ...
  do
    ...
    then
      ...
      shuf -ern $((RANDOM % (1000 - 1) + 1)) x | paste -sd ''  | cut -c 1-999 
    fi
  done
} >"$out/in"
{
timeout -s9 1 "$ref" 2>/dev/null < <(cat "$out/in"; sleep 10) >"$out/reference"
timeout -s9 1 "$bin" 2>/dev/null < <(cat "$out/in"; sleep 10) >"$out/output"
} &>/dev/null
