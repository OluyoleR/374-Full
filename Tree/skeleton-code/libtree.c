...
tree_print_recurse(struct fileinfo finfo)  
{
  ...
  if (opts.dirsonly && (!S_ISDIR(finfo.st.st_mode)) ) goto exit ;

  /* TODO: print indentation */
  for (int i = 0 ; i < (opts.indent * depth); ++i){
    putchar(' '); 
  }
...
exit:;
  ...
  //dprintf("depth:%d sav_dir:%d curr_dir:%d file_count:%ld   ",depth,sav_dir,cur_dir,file_count);
...
print_path_info(struct fileinfo finfo)
{
  ...
  if (opts.perms) {
    ...
    if (printf("%c%s", sep, perms) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (opts.user) {
    /*  Hint: getpwuid(3) */
    struct passwd *pass =  getpwuid(finfo.st.st_uid);
    const char * name = pass->pw_name;
    if (printf("%c%s", sep, name) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (opts.group) {
    /*  Hint: getgrgid(3) */
    struct group *result = getgrgid(finfo.st.st_gid);
    const char* group_name = result->gr_name;

    if (printf("%c%s", sep, group_name) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (opts.size) {
    /*  Hint: stat.h(0p) */

    if (printf("%c%jd", sep, finfo.st.st_size) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (sep != '[')
...
read_file_list(DIR *dirp, struct fileinfo **file_list, size_t *file_count)
{
  for (;;) {
    ...
    if (!(opts.all > 0 ) && de->d_name[0] == '.') continue ; 
